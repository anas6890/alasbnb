...
export async function GET() {
  try {
    // 1. Purge existing database tables completely in correct relation dependency order
    ...
    // 3. Create high-quality test Users
    // Host user
    ...
    const userWouarda = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2022-03-15"), // 4 years active
      },
    });
    ...
    const userFrancklin = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2012-06-20"), // 14 years active
      },
    });
    ...
    const userDkl = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2017-09-10"), // 9 years active
      },
    });
    ...
    const userSeverine = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2019-11-05"), // 7 years active
      },
    });
    ...
    const userNabil = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2024-04-12"), // 2 years active
      },
    });
    ...
    const userInsaf = await prisma.user.create({
      data: {
        ...
        isVerified: true,
        createdAt: new Date("2025-12-01"), // 5 months active
      },
    });

    // 4. Create premium Listings
    // Listing 1: Marrakech Villa (Matches screenshot and coordinates 100%)
    ...
    // Listing 2: Luxury Eiffel Tower Penthouse in Paris
    ...
    // 5. Create premium Experiences
    // Experience 1: Quad Desert Ride
    ...
    // 7. Create Reservations for testing
    // Active reservation for Wouarda on Marrakech Villa
    ...
    // Active reservation for Francklin on Marrakech Villa
    ...
    // Active reservation for Dkl on Marrakech Villa
    ...
    // Active reservation for Severine on Marrakech Villa
    ...
    // Active reservation for Nabil on Marrakech Villa
    ...
    // Active reservation for Insaf on Marrakech Villa
    ...
    // 8. Create premium Reviews (Commentaires) exactly matching the user's screenshots
    // Review 1: Wouarda
    await prisma.review.create({
      data: {
        authorId: userWouarda.id,
        reservationId: resWouarda.id,
        listingId: villaMarrakech.id,
        ratingCleanliness: 5,
        ratingAccuracy: 5,
        ratingCheckin: 5,
    ...
    await prisma.review.create({
      data: {
        authorId: userFrancklin.id,
        reservationId: resFrancklin.id,
        listingId: villaMarrakech.id,
        ratingCleanliness: 5,
        ratingAccuracy: 5,
        ratingCheckin: 5,
        ...
        createdAt: new Date(new Date().setDate(new Date().getDate() - 21)), // 3 weeks ago
      },
    });
    ...
    await prisma.review.create({
      data: {
        ...
        createdAt: new Date(new Date().setDate(new Date().getDate() - 14)), // 2 weeks ago
      },
    });
    ...
    await prisma.review.create({
      data: {
        authorId: userSeverine.id,
        reservationId: resSeverine.id,
        listingId: villaMarrakech.id,
        ratingCleanliness: 4,
        ratingAccuracy: 4,
        ratingCheckin: 4,
    ...
    await prisma.review.create({
      data: {
        authorId: userNabil.id,
        reservationId: resNabil.id,
        listingId: villaMarrakech.id,
        ratingCleanliness: 5,
        ratingAccuracy: 5,
        ratingCheckin: 5,
        ...
        createdAt: new Date("2026-04-12"), // April 2026
      },
    });
    ...
    await prisma.review.create({
      data: {
        authorId: userInsaf.id,
        reservationId: resInsaf.id,
        listingId: villaMarrakech.id,
        ratingCleanliness: 5,
        ratingAccuracy: 5,
        ratingCheckin: 5,
    ...
    // Review 7: Paris review
    await prisma.review.create({
      data: {
        authorId: userFrancklin.id,
        reservationId: resParis.id,
        listingId: penthouseParis.id,
        ratingCleanliness: 5,
        ratingAccuracy: 5,
        ratingCheckin: 5,
    ...
    // Review 8: Quad review
    await prisma.review.create({
      data: {
        authorId: userWouarda.id,
        reservationId: resQuad.id,
        experienceId: expQuad.id,
        avgRating: 4.9,
    ...
    return NextResponse.json({
      success: true,
      message: "Félicitations ! La base de données MongoDB a été vidée et rechargée avec succès avec des données de test extrêmement réalistes (logements d'exception, expériences typiques, vrais commentaires correspondants à vos captures d'écran, utilisateurs fictifs aux mots de passe valides, et réservations actives). Bon test !",
    });
  } catch (error: any) {
    console.error("SEED_ERROR", error);
    return NextResponse.json(
      { success: false, error: error.message || "Une erreur s'est produite lors de l'exécution de la seed." },
      { status: 500 }
    );
  }
}
